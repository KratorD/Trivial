...
use Zikula\TrivialModule\Helper\ModelHelper;
...
abstract class AbstractControllerHelper
{
    ...
    /**
     * Returns an array of all allowed object types in ZikulaTrivialModule.
     *
    ...
    public function getObjectTypes($context = '', array $args = [])
    {
        ...
        $allowedObjectTypes = [];
        $allowedObjectTypes[] = 'tournament';
        $allowedObjectTypes[] = 'question';
        $allowedObjectTypes[] = 'answer';
        $allowedObjectTypes[] = 'result';
    
        return $allowedObjectTypes;
    }

    /**
     * Returns the default object type in ZikulaTrivialModule.
     *
     * @param string $context Usage context (allowed values: controllerAction, api, helper, actionHandler, block, contentType, util)
     * @param array  $args    Additional arguments
     *
    ...
    public function processViewActionParameters($objectType, SortableColumns $sortableColumns, array $templateParameters = [], $hasHookSubscriber = false)
    {
        ...
        $templateParameters['own'] = $request->query->getInt('own', $this->variableApi->get('ZikulaTrivialModule', 'showOnlyOwnEntries', 0));
        ...
        if ($templateParameters['all'] != 1) {
            ...
            if (in_array($resultsPerPage, [0, 10])) {
                $resultsPerPage = $this->variableApi->get('ZikulaTrivialModule', $objectType . 'EntriesPerPage', 10);
            }
        }
        ...
        $quickNavForm = $this->formFactory->create('Zikula\TrivialModule\Form\Type\QuickNavigation\\' . ucfirst($objectType) . 'QuickNavType', $templateParameters);
        ...
        if (true === $hasHookSubscriber) {
            ...
            $templateParameters['currentUrlObject'] = new RouteUrl('zikulatrivialmodule_' . strtolower($objectType) . '_view', $urlParameters);
        }
    ...
    public function processDisplayActionParameters($objectType, array $templateParameters = [], $hasHookSubscriber = false)
    {
        ...
        if (true === $hasHookSubscriber) {
            ...
            $templateParameters['currentUrlObject'] = new RouteUrl('zikulatrivialmodule_' . strtolower($objectType) . '_display', $urlParameters);
        }
