...
namespace Zikula\TrivialModule\Form\Handler\Question\Base;

use Zikula\TrivialModule\Form\Handler\Common\EditHandler;
use Zikula\TrivialModule\Form\Type\QuestionType;
...
abstract class AbstractEditHandler extends EditHandler
{
    ...
    public function processForm(array $templateParameters = [])
    {
        ...
        $this->objectTypeCapital = 'Question';
        $this->objectTypeLower = 'question';
        ...
        if ($this->templateParameters['mode'] == 'create') {
            if (!$this->modelHelper->canBeCreated($this->objectType)) {
                $this->request->getSession()->getFlashBag()->add('error', $this->__('Sorry, but you can not create the question yet as other items are required which must be created before!'));
    ...
    protected function initRelationPresets()
    {
        ...
        $this->relationPresets['tournament'] = $this->request->get('tournament', '');
        if (!empty($this->relationPresets['tournament'])) {
            $relObj = $this->entityFactory->getRepository('tournament')->selectById($this->relationPresets['tournament']);
            if (null !== $relObj) {
                $relObj->addQuestions($entity);
            }
        }
    ...
    {
        return $this->formFactory->create(QuestionType::class, $this->entityRef, $this->getFormOptions());
    }
    ...
    protected function getRedirectCodes()
    {
        ...
        // user index page of question area
        $codes[] = 'userIndex';
        // admin index page of question area
        $codes[] = 'adminIndex';
        ...
        $codes[] = 'adminOwnView';
        // user detail page of treated question
        $codes[] = 'userDisplay';
        // admin detail page of treated question
        $codes[] = 'adminDisplay';
    
        // user list of tournaments
        $codes[] = 'userViewTournaments';
        // admin list of tournaments
        $codes[] = 'adminViewTournaments';
        // user list of own tournaments
        $codes[] = 'userOwnViewTournaments';
        // admin list of own tournaments
        $codes[] = 'adminOwnViewTournaments';
    ...
    protected function getDefaultMessage(array $args = [], $success = false)
    {
        ...
        switch ($args['commandName']) {
            case 'submit':
                if ($this->templateParameters['mode'] == 'create') {
                    $message = $this->__('Done! Question created.');
                } else {
                    $message = $this->__('Done! Question updated.');
                }
                break;
            case 'delete':
                $message = $this->__('Done! Question deleted.');
                break;
            default:
                $message = $this->__('Done! Question updated.');
                break;
    ...
    public function applyAction(array $args = [])
    {
        ...
        try {
            ...
            $logArgs = ['app' => 'ZikulaTrivialModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => 'question', 'id' => $entity->getKey(), 'errorMessage' => $exception->getMessage()];
    ...
    protected function getRedirectUrl(array $args = [])
    {
        ...
        switch ($this->returnTo) {
            ...
            case 'adminDisplay':
                ...
                return $this->getDefaultReturnUrl($args);
            case 'userViewTournaments':
            case 'adminViewTournaments':
                return $this->router->generate('zikulatrivialmodule_tournament_' . $routeArea . 'view');
            case 'userOwnViewTournaments':
            case 'adminOwnViewTournaments':
                return $this->router->generate('zikulatrivialmodule_tournament_' . $routeArea . 'view', ['own' => 1]);
            default:
                return $this->getDefaultReturnUrl($args);
        }
    }
