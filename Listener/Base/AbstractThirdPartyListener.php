...
abstract class AbstractThirdPartyListener implements EventSubscriberInterface
{
     ...
     * ZikulaTrivialModule will use this to add a javascript helper to add custom items.
     *
    ...
    public function getEditorHelpers(EditorHelperEvent $event)
    {
        ...
        $targetDir = 'web/modules/zikulatrivial';
        $finder = new Finder();
        if (!$this->filesystem->exists($targetDir)) {
            $this->filesystem->mkdir($targetDir, 0777);
            if (is_dir($originDir = 'modules/Zikula/TrivialModule/Resources/public')) {
                $this->filesystem->mirror($originDir, $targetDir, Finder::create()->in($originDir));
            }
            if (is_dir($originDir = 'modules/Zikula/TrivialModule/Resources/scribite')) {
                $targetDir .= '/scribite';
                $this->filesystem->mkdir($targetDir, 0777);
                $this->filesystem->mirror($originDir, $targetDir, Finder::create()->in($originDir));
            }
        ...
        $event->getHelperCollection()->add(
            [
                ...
                'type' => 'javascript',
                'path' => $this->request->getBasePath() . '/web/modules/zikulatrivial/js/ZikulaTrivialModule.Finder.js'
            ]
        );
    }
    ...
    public function getCKEditorPlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'zikulatrivialmodule',
            'path' => $this->request->getBasePath() . '/web/modules/zikulatrivial/scribite/CKEditor/zikulatrivialmodule/',
            'file' => 'plugin.js',
            'img' => 'ed_zikulatrivialmodule.gif'
        ]);
    }
    ...
    public function getQuillPlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'zikulatrivialmodule',
            'path' => $this->request->getBasePath() . '/web/modules/zikulatrivial/scribite/Quill/zikulatrivialmodule/plugin.js'
        ]);
    }
    ...
    public function getSummernotePlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'zikulatrivialmodule',
            'path' => $this->request->getBasePath() . '/web/modules/zikulatrivial/scribite/Summernote/zikulatrivialmodule/plugin.js'
        ]);
    }
    ...
    public function getTinyMcePlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'zikulatrivialmodule',
            'path' => $this->request->getBasePath() . '/web/modules/zikulatrivial/scribite/TinyMce/zikulatrivialmodule/plugin.js'
        ]);
    }
}
