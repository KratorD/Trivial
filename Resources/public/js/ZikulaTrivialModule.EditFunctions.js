 ...
 */
function zikulaTrivialInitDateField(fieldName) {
...
function zikulaTrivialTriggerFormValidation() {
    zikulaTrivialExecuteCustomValidationConstraints(editedObjectType, editedEntityId);
...
function zikulaTrivialHandleFormSubmit (event) {
    if (triggerValidation) {
        zikulaTrivialTriggerFormValidation();
        if (!editForm.get(0).checkValidity()) {
            event.preventDefault();
            return false;
        }
 ...
 */
function zikulaTrivialInitEditForm(mode, entityId) {
    if (jQuery('.zikulatrivial-edit-form').length < 1) {
        return;
    }

    editForm = jQuery('.zikulatrivial-edit-form').first();
    ...
    editForm.submit(zikulaTrivialHandleFormSubmit);
