...
<div class="row">
    <div class="col-sm-8">
        ...
        {if $properties ne null && is_array($properties)}
            ...
            {foreach key='propertyName' item='propertyId' from=$properties}
                <div class="form-group">
                    {assign var='hasMultiSelection' value=$categoryHelper->hasMultipleSelection('question', $propertyName)}
                    ...
                    <div class="col-sm-9">
                        {selector_category name="`$baseID`_`$categorySelectorName``$propertyName`" field='id' selectedValue=$catIds.$propertyName|default:null categoryRegistryModule='ZikulaTrivialModule' categoryRegistryTable="`$objectType`Entity" categoryRegistryProperty=$propertyName defaultText=$lblDefault editLink=false multipleSize=$categorySelectorSize cssClass='form-control'}
                    </div>
                </div>
            {/foreach}
            {/nocache}
        {/if}
        <div class="form-group">
            <label for="{$baseID}Id" class="col-sm-3 control-label">{gt text='Question'}:</label>
            <div class="col-sm-9">
                <select id="{$baseID}Id" name="id" class="form-control">
                    {foreach item='question' from=$items}
                        <option value="{$question->getKey()}"{if $selectedId eq $question->getKey()} selected="selected"{/if}>{$question->getQuestion()}</option>
                    {foreachelse}
        ...
        <div class="form-group">
            ...
            <div class="col-sm-9">
                <select id="{$baseID}Sort" name="sort" class="form-control">
                    <option value="question"{if $sort eq 'question'} selected="selected"{/if}>{gt text='Question'}</option>
                    <option value="corrAnswer"{if $sort eq 'corrAnswer'} selected="selected"{/if}>{gt text='Corr answer'}</option>
            ...
            <div class="col-sm-9">
                <div class="input-group">
                    ...
                    <span class="input-group-btn">
                        <input type="button" id="zikulaTrivialModuleSearchGo" name="gosearch" value="{gt text='Filter'}" class="btn btn-default" />
                    </span>
                </div>
            </div>
        </div>
