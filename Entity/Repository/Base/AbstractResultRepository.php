 ...
 *
 * This is the base repository class for result entities.
 */
abstract class AbstractResultRepository extends EntityRepository
{
    /**
     * @var string The main entity class
     */
    protected $mainEntityClass = 'Zikula\TrivialModule\Entity\ResultEntity';

    /**
     * @var string The default sorting field/expression
     */
    protected $defaultSortingField = 'score';
    ...
    public function updateCreator($userId, $newUserId, TranslatorInterface $translator, LoggerInterface $logger, CurrentUserApiInterface $currentUserApi)
    {
        ...
        $logArgs = ['app' => 'ZikulaTrivialModule', 'user' => $currentUserApi->get('uname'), 'entities' => 'results', 'userid' => $userId];
    ...
    public function updateUserField($userFieldName, $userId, $newUserId, TranslatorInterface $translator, LoggerInterface $logger, CurrentUserApiInterface $currentUserApi)
    {
        ...
        if (empty($userFieldName) || !in_array($userFieldName, ['player'])) {
        ...
        $logArgs = ['app' => 'ZikulaTrivialModule', 'user' => $currentUserApi->get('uname'), 'entities' => 'results', 'field' => $userFieldName, 'userid' => $userId, 'newuserid' => $newUserId];
    ...
    public function deleteByUserField($userFieldName, $userId, TranslatorInterface $translator, LoggerInterface $logger, CurrentUserApiInterface $currentUserApi)
    {
        ...
        $logArgs = ['app' => 'ZikulaTrivialModule', 'user' => $currentUserApi->get('uname'), 'entities' => 'results', 'field' => $userFieldName, 'userid' => $userId];
     ...
     *
     * @return array|resultEntity Retrieved data array or resultEntity instance
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false)
     *
     * @return ArrayCollection Collection containing retrieved resultEntity instances
     */
    ...
    public function getListQueryBuilder($where = '', $orderBy = '', $useJoins = true, $slimMode = false)
    {
        ...
        if ((!$useJoins || !$slimMode) && null !== $this->collectionFilterHelper) {
            $qb = $this->collectionFilterHelper->addCommonViewFilters('result', $qb);
        }
    ...
    public function selectSearch($fragment = '', array $exclude = [], $orderBy = '', $currentPage = 1, $resultsPerPage = 25, $useJoins = true)
    {
        ...
        if (null !== $this->collectionFilterHelper) {
            $qb = $this->collectionFilterHelper->addSearchFilter('result', $qb, $fragment);
        }
    ...
    {
        $selection = 'COUNT(tbl.id) AS numResults';
        if (true === $useJoins) {
    ...
    public function selectCount($where = '', $useJoins = false, array $parameters = [])
    {
        ...
        if (null !== $this->collectionFilterHelper) {
            $qb = $this->collectionFilterHelper->applyDefaultFilters('result', $qb, $parameters);
        }
     ...
     * @param integer $excludeId  Id of results to exclude (optional)
     *
     * @return boolean Result of this check, true if the given result does not already exist
     */
